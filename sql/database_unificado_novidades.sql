-- =========================================================
-- DOITLY - Pacote unificado de novidades de banco
-- Inclui catálogo completo de conquistas e correções recentes
-- =========================================================

START TRANSACTION;

-- Garante estrutura mínima esperada da tabela achievements.
ALTER TABLE `achievements`
    MODIFY `slug` varchar(50) NOT NULL,
    MODIFY `name` varchar(100) NOT NULL,
    MODIFY `description` text NOT NULL,
    MODIFY `icon` varchar(50) DEFAULT NULL,
    MODIFY `badge_color` varchar(7) DEFAULT '#FFD700',
    MODIFY `criteria_type` enum('streak','total_completions','habits_count','perfect_week','perfect_month') NOT NULL,
    MODIFY `criteria_value` int(10) UNSIGNED NOT NULL,
    MODIFY `points` int(10) UNSIGNED NOT NULL DEFAULT 0,
    MODIFY `rarity` enum('common','rare','epic','legendary') NOT NULL DEFAULT 'common',
    MODIFY `is_active` tinyint(1) NOT NULL DEFAULT 1;


-- Catálogo canônico de conquistas (18 registros).
