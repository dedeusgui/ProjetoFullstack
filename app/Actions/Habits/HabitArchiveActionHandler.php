...
final class HabitArchiveActionHandler
{
    public function handle(\mysqli $conn, array $post, array $server, array &$session): ActionResponse
    {
        ...
        $habitId = (int) ($post['habit_id'] ?? 0);
        $operation = (string) ($post['operation'] ?? 'archive');

        $service = new HabitCommandService($conn);
        $result = $operation === 'restore'
            ? $service->restore($userId, $habitId)
            : $service->archive($userId, $habitId);
